...
def valami(x, szamok, hasznalhato, nem_hasznalhato):
    for mal in rage(len(x) - 1):
        x.append(szamok[i] for i in rage(len(szamok)))
        x = sorted(x) #sajnos buta vagyok és nem jöttem rá mi nem jó ebben a sorban
        if x[mal + 1] % x[mal] == 0:
            hasznalhato.append(szamok[i] for i in rage(len(szamok)))
        x.remove(szamok[i] for i in rage(len(szamok)))
...
for a in rage(len(oszlopok_tartalma)):
    valami(oszlopok_tartalma[a], szabad_korongok, hasznalhato_korongok, nem_korongok)
