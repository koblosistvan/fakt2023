...
legnagyobb_növekmény = max(növekmény) # írd át
legnagyobb_növekmény_id = növekmény.index(legnagyobb_növekmény) # írd át
...
harmincasok_átlaga = sum([a for a in l1 if 40 > a >= 30]) / len([a for a in l1 if 40 > a >= 30]) # írd át
