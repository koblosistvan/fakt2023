...
for i in range(len(munkaxp)-1):
	if munkaxp[i] > munkaxp[i+1]:
		csesetek += 1
		csfizetes.append(fizetes[i], fizetes[i+1])
		csmunkaxp.append(munkaxp[i], munkaxp[i+1])
...
magasfizetes = munkaxp[magasfizetesindex].split('.')
...
for i in range(len(fizetes)):
	print print(f"{csmunkaxp[a]} -> {csmunkaxp[b]} év: ${csfizetes[a]} -> ${csfizetes[b]} ({csfizetes[b]-csfizetes[a]})")
	a += 2
	b += 2
