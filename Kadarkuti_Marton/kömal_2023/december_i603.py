...
# bemenet: "a" szám sub_a eleme, "b" szám sub_b eleme
    # a és b közötti oszthatósági kapcsolatot keressük
# return értékek:
    # 1, ha a|b
    # 1, ha a=b, mert a definíció alapján a számjegyek egyezését is vizsgáljuk
                    # a=b=0 érvényes, mert a nullára itt számjegyként és nem osztóként tekintünk.
    # 0, ha a∤b, ha b=0, vagy ha b nem egyjegyű,
                        # mert a példa alapján az osztó mindig egy számjegy

def d(a,b):
    if len(str(b)) > 1:  # számjegyeket keresünk
        return 0
    if a == b: # két számjegy egyezése mindig érvényes
        return 1
    if b == 0: # zéróosztó
        return 0
    if a%b == 0:
