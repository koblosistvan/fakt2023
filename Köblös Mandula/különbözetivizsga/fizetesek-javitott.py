...
for sor in forrás:
    ...
    munkatapasztalat.append(adat[0])
...
legmagasabb = kereslet[0]
legmagasabb_index = 0
for i in range(len(munkatapasztalat)):
    if kereslet[i] > legmagasabb:
        legmagasabb = kereslet[i]
        legmagasabb_index = i
print(f"a Legmagasabb fizetést {munkatapasztalat[legmagasabb_index]} év munkatapasztalattal lehet kapni.")
